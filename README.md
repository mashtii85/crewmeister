...
A project to see list of absence on crewmeister and their reasons plus number of days they reqeusted and whether they got confirmation, or rejection.

I was trying to obey seperation of concern by dividing functionalites between projects using mono repo approach.

## Stack

Nx mono repo, Nextjs, react query, tailwind, styled-components, eslint, prettier, prettier, cypress, jest, typescript

## How it works

### Instalation

The fist step is to install packages using yarn or npm.

- Run `npm i` or `yarn`

You can choose one of the above package manager of your choice to install packages

### Run

By running npm start project started and you can see list of absence chained to the correspondent members
We can sort absences by name and type both in acending and descending orders. On action column we can see details of each member on an Off Canvas.
...
For Testing application, I went for cypress, you may ask why? Short answer would be you can write end-to-end, Integration and unit tests. You are just need to give it a try, and you are going to love it. here's coprehensive answer to the why, https://docs.cypress.io/guides/overview/why-cypress#What-you-ll-learn
...
- crewmeister
- crewmeister-e2e
  We have two front-end projects, crewmeister and it's correspondent e2e project. The crewmeister is where the stroy begins.
...
If we want to have a helper to let for example a currency, comma separated, then the comma-separated function is a utility and it would lives in this library. So any utility lives in this library.
...
### Do we really need a mono repo and different libraries
...
Every small project is going to be a beast(gigantic) some day, one day or at least let's think in this way. So if from the very begining we have the structure and core which has the ability to expand and grow then we are in business.
...
I used to work with Material UI and IUI lately. Then I heard tailwind is on the market and has a lot to say. So I decided to give it a shot and it turns out I love it. I should say, yes it was the first time I was using tailwind

### react-query

I think it's a must for anyone who works with API to about react-query. we also have SWR, but I love the way react query works. I used apollo client and it's kind of like that
...
it's a must that every application should follow the same pattern and I think with prettier and linter we can reach this goal. for linting all projects and libs:
